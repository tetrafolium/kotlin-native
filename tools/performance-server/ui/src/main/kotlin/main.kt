...
fun getChartData(
    labels: List<String>,
    valuesList: Collection<List<*>>,
    stageToShow: Int = 0,
    buildsNumber: Int? = null,
        ...
        { data ->
            ...
            if (data.type == "point") {
                ...
                if (currentBuild.failuresNumber > 0) {
                    ...
                    svgParameters["d"] = arrayOf(
                        "M", data.x, data.y - pointSize,
                        "L", data.x - pointSize, data.y + pointSize / 2,
                        "L", data.x + pointSize, data.y + pointSize / 2, "z"
                    ).joinToString(" ")
                ...
                element._node.addEventListener(
                    "click",
                    {
...
fun main(args: Array<String>) {
    ...
    // Draw charts.
    val execChart = Chartist.Line(
        "#exec_chart", getChartData(labels, executionTime.values, stageToShow, buildsNumberToShow),
        getChartOptions(executionTime.keys.toTypedArray(), "Normalized time")
    )
    val compileChart = Chartist.Line(
        "#compile_chart", getChartData(labels, compileTime.values, stageToShow, buildsNumberToShow),
        getChartOptions(compileTime.keys.toTypedArray(), "Time, milliseconds")
    )
    val codeSizeChart = Chartist.Line(
        "#codesize_chart", getChartData(labels, codeSize.values, stageToShow, buildsNumberToShow, sizeClassNames),
        getChartOptions(codeSize.keys.toTypedArray(), "Normalized size", arrayOf("ct-series-4", "ct-series-5", "ct-series-6"))
    )
    val bundleSizeChart = Chartist.Line(
        "#bundlesize_chart", getChartData(labels, listOf(bundleSize), stageToShow, buildsNumberToShow, sizeClassNames),
