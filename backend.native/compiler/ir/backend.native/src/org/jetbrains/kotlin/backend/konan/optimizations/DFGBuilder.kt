        ...
        irModule.accept(
            object : IrElementVisitorVoid {
                    ...
                    if (declaration.parent is IrFile)
                        declaration.initializer?.let {
                            ...
                            analyze(
                                declaration,
                                IrSetFieldImpl(
                                    it.startOffset, it.endOffset, declaration.symbol, null,
                ...
                private fun analyze(declaration: IrDeclaration, body: IrElement?) {
                    ...
                    val function = FunctionDFGBuilder(
                        expressionValuesExtractor, visitor.variableValues,
                        declaration, visitor.expressions, visitor.returnValues, visitor.thrownValues, visitor.catchParameters
                    ).build()
    ...
    private inner class ElementFinderVisitor : IrElementVisitorVoid {
        ...
        override fun visitExpression(expression: IrExpression) {
            ...
            if (expression is IrCall && expression.symbol == executeImplSymbol) {
                ...
                val producerInvocation = IrCallImpl(
                    expression.startOffset, expression.endOffset,
                        ...
                        when (value) {
                            ...
                            is IrCall -> when (value.symbol) {
                                ...
                                in arrayGetSymbols -> {
                                    val callee = value.symbol.owner
                                    val actualCallee = (
                                        value.superQualifierSymbol?.owner?.getOverridingOf(callee)
                                            ?: callee
                                        ).target
...
//                                                        }
                            ...
                            is IrDelegatingConstructorCall -> {
                                ...
                                val thiz = IrGetValueImpl(
                                    SYNTHETIC_OFFSET, SYNTHETIC_OFFSET, thisReceiver.type,
                                    thisReceiver.symbol
                                )
