...
internal class ExternalModulesDFG(
...
internal object DFGSerializer {
    ...
    class TypeBase(val isFinal: Boolean, val isAbstract: Boolean, val primitiveBinaryType: PrimitiveBinaryType?, val name: String?) {
        ...
        constructor(data: ArraySlice) : this(
    ...
    class DeclaredType(
        val base: TypeBase,
        val index: Int,
        val superTypes: IntArray,
        val vtable: IntArray,
        val itable: Array<ItableSlot>
    ) {

        constructor(data: ArraySlice) : this(
            TypeBase(data), data.readInt(), data.readIntArray(),
    ...
    class FunctionSymbolBase(
        val parameters: Array<FunctionParameter>,
        val returnParameter: FunctionParameter,
        val attributes: Int,
        val escapes: Int?,
        val pointsTo: IntArray?
    ) {

        constructor(data: ArraySlice) : this(
            data.readArray { FunctionParameter(data) }, FunctionParameter(data), data.readInt(),
    ...
    class FunctionSymbol(
        val base: FunctionSymbolBase,
        val external: ExternalFunctionSymbol?,
        val public: PublicFunctionSymbol?,
        val private: PrivateFunctionSymbol?
    ) {
        ...
        val functionSymbols = functionSymbolMap.entries
            ...
            .map {
                ...
                fun buildFunctionParameter(parameter: DataFlowIR.FunctionParameter) =
                    FunctionParameter(
                        typeMap[parameter.type]!!, parameter.boxFunction?.let { functionSymbolMap[it]!! },
                ...
                when (symbol) {
                    ...
                    is DataFlowIR.FunctionSymbol.Public ->
                        FunctionSymbol.public(
                            buildFunctionSymbolBase(symbol), symbol.hash,
                            symbol.symbolTableIndex, bridgeTarget, symbol.name
                        )

                    is DataFlowIR.FunctionSymbol.Private ->
                        FunctionSymbol.private(
                            buildFunctionSymbolBase(symbol), symbol.symbolTableIndex,
            ...
            if (libraryDataFlowGraph != null) {
                ...
                val types = symbolTable.types.map {
                    if (it.virtual)
                        ...
                        when {
                            external != null ->
                                DataFlowIR.Type.External(
                                    external.hash, external.base.isFinal, external.base.isAbstract,
                            ...
                            public != null -> {
                                ...
                                DataFlowIR.Type.Public(
                                    public.hash, -1 /*TODO*/, public.intestines.base.isFinal,
                                    public.intestines.base.isAbstract, public.intestines.base.primitiveBinaryType,
                                    module, symbolTableIndex, null, public.intestines.base.name
                                ).also {
                                ...
                                if (symbolTableIndex >= 0)
                                    ++module.numberOfClasses
                                DataFlowIR.Type.Private(
                                    privateTypeIndex++, private.intestines.base.isFinal,
                                    private.intestines.base.isAbstract, private.intestines.base.primitiveBinaryType,
                                    module, symbolTableIndex, null, private.intestines.base.name
                                ).also {
                                    allTypes += it
                                }
                            }
                ...
                val functionSymbols = symbolTable.functionSymbols.map {
                    ...
                    when {
                        ...
                        public != null -> {
                            ...
                            DataFlowIR.FunctionSymbol.Public(
                                public.hash,
                                module, symbolTableIndex, attributes, null, null, public.name
                            ).also {
                            ...
                            if (symbolTableIndex >= 0)
                                ++module.numberOfFunctions
                            DataFlowIR.FunctionSymbol.Private(
                                privateFunIndex++,
                ...
                fun buildFunctionParameter(parameter: FunctionParameter) =
                    DataFlowIR.FunctionParameter(
                        types[parameter.type], parameter.boxFunction?.let { functionSymbols[it] },
